...
type Float64GorillaCompress struct {
	lastVal  uint64
	leading  uint8
	trailing uint8
}

func NewFloat64Compress(val uint64, bw *BBuffer) *Float64GorillaCompress {
	bw.WriteBits(val, 64)
	return &Float64GorillaCompress{
		lastVal: val,
	}
}

func (c *Float64GorillaCompress) Compress(val uint64, bw *BBuffer) {
	xor := val ^ c.lastVal
	if xor == 0 {
		bw.WriteBit(Zero)
	} else {
...
type Float64GorillaDecompress struct {
	val      *uint64
	leading  uint8
	trailing uint8
}

func NewFloat64Decompress(br *BitReader, ptr *uint64) (*Float64GorillaDecompress, error) {
	val, err := br.ReadBits(64)
	if err != nil {
		return nil, err
	}
	*ptr = val
	return &Float64GorillaDecompress{
		val:      ptr,
		leading:  0,
		trailing: 0,
	}, nil
...
func (d *Float64GorillaDecompress) Decompress(br *BitReader) error {
...
func (d *Float64GorillaDecompress) ToFloat64Compress() *Float64GorillaCompress {
	return &Float64GorillaCompress{
		lastVal:  *d.val,
		leading:  d.leading,
		trailing: d.trailing,
